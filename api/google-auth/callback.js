...
module.exports = async function handler(req, res) {
  ...
  if (error || !code) return res.status(400).send(`<h2>인증 실패: ${error || '코드 없음'}</h2>`);
  ...
  try {
    ...
    res.send(`<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>연동 완료</title>
      ...
      button:hover{background:#4f46e5}p{margin:6px 0;font-size:14px}
      ...
      <div class="card green"><p><strong>📋 복사할 Refresh Token</strong></p>
        ${refreshToken
          ...
          : `<p class="warn">⚠️ Refresh Token이 없습니다. <a href="/api/google-auth">여기서 재인증</a>하세요.</p>`}
      </div>
      <div class="card blue"><p><strong>📌 Vercel에 등록하는 방법</strong></p>
        <div class="step"><div class="num">1</div><p>Vercel → juganbogo3 → <b>Settings → Environment Variables</b></p></div>
        <div class="step"><div class="num">2</div><p>Key: <b>GOOGLE_REFRESH_TOKEN</b> / Value: 위 토큰 붙여넣기 → Add</p></div>
