...
async function getOrCreateFolder(drive, name, parentId) {
  ...
  const created = await drive.files.create({
    requestBody: { name, mimeType: 'application/vnd.google-apps.folder', parents: parentId ? [parentId] : (process.env.GOOGLE_DRIVE_FOLDER_ID ? [process.env.GOOGLE_DRIVE_FOLDER_ID] : []) },
    fields: 'id',
  });
      ...
      children.push(new Table({
        ...
        rows: item.tableData.map((row,rIdx)=>{
          ...
          row.forEach((cellText,cIdx)=>{
            ...
            cells.push(new TableCell({children:[new Paragraph({children:[new TextRun({text:cellText||'',bold:isH,color:isH?'1D4ED8':'000000',font:'맑은 고딕',size:18})]})],columnSpan:colSpan,rowSpan,shading:isH?{fill:'EFF6FF'}:undefined}));
          });
          return new (require('docx').TableRow)({children:cells});
        }),
      }));
    }
  }
...
function buildTextContent(parish, church, payload) {
  ...
  for (const item of (payload.data||[])) {
    ...
    if(item.tableData?.length>0){
      txt+='\n[데이터 테이블]\n';
      item.tableData.forEach((row,rIdx)=>{txt+='| '+row.map(c=>c.trim().replace(/\n/g,' ')).join(' | ')+' |\n';if(rIdx===0)txt+='| '+row.map(()=>'---').join(' | ')+' |\n';});
      txt+='\n';
    }
  }
  return txt;
...
module.exports = async function handler(req, res) {
  ...
  const parish=parts[1], church=parts[2];
  ...
  try {
    ...
    if (payload?.status==='submitted') {
      ...
      await uploadOrUpdate(drive,`${cleanChurch}_주간보고_${new Date().toISOString().slice(0,10)}.docx`,wordBuffer,'application/vnd.openxmlformats-officedocument.wordprocessingml.document',parishFolderId);
    }
